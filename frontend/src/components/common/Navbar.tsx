...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#EDEAFD] border-b border-[#E5E4E8]/60 backdrop-blur-md">
      <div className="flex h-16 items-center justify-between px-6 max-w-7xl mx-auto">
        {/* Left: Logo */}
        <Logo size="md" href={isWarden ? '/warden' : '/student'} />

        {/* Center: Top Pill Navigation Bar */}
        <div className="hidden md:flex items-center gap-1.5 bg-white border border-[#E5E4E8] rounded-full px-3 py-1.5 shadow-sm text-xs font-medium text-[#3C315B]">
          {isWarden ? (
            <>
              <Link
                href="/warden"
                className={`px-3.5 py-1 rounded-full transition-colors flex items-center gap-1 ${
                  pathname === '/warden' ? 'bg-[#ECE8FE] font-bold text-[#3C315B]' : 'hover:bg-[#F4F2F8]'
                }`}
              >
                Console <ChevronDown className="w-3 h-3 opacity-60" />
              </Link>
              <Link
                href="/warden/rooms"
                className={`px-3.5 py-1 rounded-full transition-colors flex items-center gap-1 ${
              ...
              >
                Rooms <ChevronDown className="w-3 h-3 opacity-60" />
              </Link>
              <Link
              ...
              >
                Students <ChevronDown className="w-3 h-3 opacity-60" />
              </Link>
              <Link
                href="/warden/tickets"
                className={`px-3.5 py-1 rounded-full transition-colors flex items-center gap-1 ${
              ...
              >
                Tickets <ChevronDown className="w-3 h-3 opacity-60" />
              </Link>
            </>
          ) : (
            <>
              ...
              >
                Bookings <ChevronDown className="w-3 h-3 opacity-60" />
              </Link>
              <Link
